...
拆分 s15 时，我先顺着一次请求把代码走了一遍：输入交给谁，模型在哪里调用，工具由谁执行，结果又怎么回来。理清这条主线后，再把任务、记忆、团队等功能拆成独立模块，最后由 `Harness` 把它们接起来。这样既能看清整个程序怎么运行，修改某项功能时也容易找到对应的代码。
...
拆分时，我也给每份状态找了明确的归属：任务由 `TaskStore` 管，审批记录由 `TeamsManager` 管，provider 和 MCP 各自管理连接，具体会话保存自己的消息历史。以后加功能，先想清楚数据归谁维护，就比较容易判断代码该放在哪里。
...
下面保留主循环的执行顺序。逐步对照源码、查看消息数据如何变化，可以读 [完整工作流程](workflow.md)，其中用一次读取文件的请求串起了输入、模型调用、工具执行和会话保存。
...
我把目录释放放在整组工具执行完之后，是为了处理这种情况：模型在同一响应中要求“完成任务，再读取刚写好的文件”。如果完成任务时立刻改掉 cwd，下一条读取就会落到主仓库。整组工具使用同一个任务目录，就能避免这个问题；owner 在释放目录前也不能再领取第二个任务。
...
当前按**一个 workspace 运行一个 Harness 宿主**设计。任务和邮箱通过跨进程锁保护文件读写；cron 文件只支持一个宿主写入，各服务之间还没有多宿主协调机制。
...
上下文管理负责控制发给模型的信息量，完整资料则另外保存，方便回查。工具调用和结果必须成组处理，不能裁掉调用却留下孤立结果。压缩顺序为：大输出落盘/预算 → 中段归档 → 旧结果缩减 → 摘要；超出模型实际窗口时再进行一次响应式压缩。字符预算用于估算长度，实际 token 数由模型接口决定。
...
后续如果要接数据库、远程队友、Web UI 或操作系统沙箱，可以从这些接口继续扩展。对我来说，这次拆分最直接的收获，是能沿着一轮请求找到每个模块，也能在改动之前判断它会影响哪些部分。
